(function () {
    ...
    function applyZoom() {
        const zoomTarget = tableContainer ? tableContainer.querySelector("#priceTableZoomTarget") : null;
        if (!zoomTarget) {
            return;
        }
    ...
    function updateResultsSummary(visibleCount, filters = {}) {
        ...
        const label = totalCount === visibleCount
    ...
    function populateBrandFilterOptions(items) {
        ...
        brands.sort((a, b) => a.localeCompare(b, "uk", { sensitivity: "base" }));
        ...
        if (previousValue && brands.some((brand) => brand.toLocaleLowerCase("uk") === previousValue.toLocaleLowerCase("uk"))) {
            const matchedBrand = brands.find((brand) => brand.toLocaleLowerCase("uk") === previousValue.toLocaleLowerCase("uk"));
    ...
    function updateActiveFilters(filters) {
        ...
        const active = [];
        const nameValue = typeof filters?.name === "string" ? filters.name.trim() : "";
        const brandLabel = typeof filters?.brandLabel === "string" ? filters.brandLabel.trim() : "";
    ...
    function renderItems(items, filters = {}) {
        ...
        if (!hasItems) {
            ...
            items.forEach((item) => {
                ...
                const priceValue = normalisePrice(item?.price ?? item?.priceText);
                if (priceValue) {
                ...
                const metaRows = [
                    ...
                    createMetaRow("ID", item?.id != null && item.id !== "" ? String(item.id) : null)
                ].filter(Boolean);
    ...
    function createSorter(key) {
        return (a, b) => {
            ...
            return aValue.toString().localeCompare(bValue.toString(), "uk", { sensitivity: "base" });
        };
    }
    ...
    function applyFilters() {
        ...
        const brandLabel = brandValue
            ? (brandFilterSelect && brandFilterSelect.selectedIndex >= 0
                ? brandFilterSelect.options[brandFilterSelect.selectedIndex]?.textContent || brandValue
                : brandValue)
            : "";
        ...
        if (nameQuery) {
            const normalisedQuery = nameQuery.toLocaleLowerCase("uk");
            filtered = filtered.filter((item) => {
                const name = item?.name;
                if (name == null) {
                    return false;
                }
                return name.toString().toLocaleLowerCase("uk").includes(normalisedQuery);
            });
        }
