(function () {
    ...
    const representativesCookieName = section?.dataset?.representativesCookie || 'salesPlanRepresentatives';
    ...
    const sendExportRequest = async () => {
        ...
        const originalText = downloadDataBtn.dataset.originalText || downloadDataBtn.textContent.trim();
        ...
        try {
            ...
            if (!response.ok || payload.error) {
                const message = payload?.error || 'Не вдалося завантажити дані';
            ...
            downloadDataBtn.disabled = false;
            downloadDataBtn.textContent = downloadDataBtn.dataset.originalText || 'Завантажити дані';
        }
    };
    ...
    if (userRole === 'MANAGER') {
        ...
        const renderSummary = (data) => {
            ...
            const remainingAmount = Number(data.remainingAmount ?? Math.max(overallPlan - totalExecution, 0));
            const overdueDebtAmount = Number(data.overdueDebtAmount ?? 0);
            const overdueDebtPercent = Number(data.overdueDebtPercent ?? (overallPlan > 0
            ...
            const remainingDays = Number(data.remainingWorkDays ?? Math.max(totalWorkDays - workedDays, 0));
            const workedRatio = Number(data.workedRatio ?? (totalWorkDays > 0 ? workedDays / totalWorkDays : 0));
            const workedPercent = Number(data.workedPercent ?? workedRatio * 100);

            const percent = Number(data.percent ?? (overallPlan > 0 ? (totalExecution * 100) / overallPlan : 0));
            const forecastAmount = workedRatio > 0 ? totalExecution / workedRatio : totalExecution;
            const forecastPercent = overallPlan > 0 ? (forecastAmount * 100) / overallPlan : 0;
            const dailyNeed = remainingDays > 0
                ? Math.max(overallPlan - totalExecution, 0) / remainingDays
                : 0;

            $('overallPlan').textContent = formatCurrency(overallPlan);
            $('totalExecution').textContent = formatCurrency(totalExecution);
            $('remainingAmount').textContent = formatCurrency(remainingAmount);
            $('percent').textContent = `${percent.toFixed(1)}%`;
            $('progressBar').style.width = `${Math.min(percent, 100).toFixed(1)}%`;
            ...
            $('tableForecastPercent').textContent = `${forecastPercent.toFixed(1)}%`;
            $('tableRemaining').textContent = fmt(remainingAmount);
            $('tableDailyNeed').textContent = fmt(dailyNeed);
            $('tableDebtPercent').textContent = `${overdueDebtPercent.toFixed(1)}%`;
        ...
        const filterContainer = document.getElementById('representativesFilter');
        const optionsContainer = document.getElementById('representativesOptions');
        const summaryEl = document.getElementById('representativesSummary');
        const selectAllBtn = document.getElementById('representativesSelectAll');
        const fetchReportBtn = document.getElementById('representativesFetchReport');
        ...
        const normalizeSelection = (values) => {
            if (!Array.isArray(values) || !values.length || !availableRepresentatives.length) {
                return [];
            }
            ...
            values.forEach((value) => {
                ...
                if (!trimmed || !availableSet.has(trimmed) || normalized.includes(trimmed)) {
                    return;
                }
        ...
        const readSelectionFromCookie = () => {
            ...
            try {
                ...
                console.warn('Не вдалося прочитати кукі з представниками', err);
                return [];
            }
        };
        ...
        const writeSelectionToCookie = (selection) => {
            ...
            const finalSelection = normalized.length ? normalized : [...availableRepresentatives];
            setCookieValue(representativesCookieName, JSON.stringify(finalSelection));
        ...
        const updateSummaryLabel = (selection) => {
            ...
            if (!selection.length || selection.length === availableRepresentatives.length) {
        ...
        const updateFetchButtonState = () => {
            ...
            const hasSelection = availableRepresentatives.length > 0 && getSelectedFromDom().length > 0;
            fetchReportBtn.disabled = isFetchingReport || !hasSelection;
        };

        const renderRepresentatives = (representatives, initialSelection) => {
            ...
            representatives.forEach((name, index) => {
                ...
                checkbox.addEventListener('change', () => {
                    ...
                    const finalSelection = writeSelectionToCookie(selectedValues);
        ...
        const fetchRepresentatives = async () => {
            ...
            try {
                const response = await fetch('/api/sales/plan/representatives', {
                ...
                if (!response.ok || !payload) {
                    throw new Error(payload?.error || 'Не вдалося отримати список представників');
                }
        ...
        const loadTable = async () => {
            ...
            try {
                const { isJson, payload } = await requestPlanData();
                if (isJson) {
                    const message = payload?.error || 'Не вдалося отримати звіт';
                    throw new Error(message);
                }
        ...
        const fetchReport = async () => {
            ...
            if (fetchReportBtn) {
                const originalText = fetchReportBtn.dataset.originalText || fetchReportBtn.textContent.trim();
            ...
            try {
                ...
                if (fetchReportBtn) {
                    fetchReportBtn.textContent = fetchReportBtn.dataset.originalText || 'Отримати звіт';
                }
                updateFetchButtonState();
            }
        };
        ...
        if (selectAllBtn) {
            selectAllBtn.addEventListener('click', (event) => {
                ...
                const finalSelection = writeSelectionToCookie([...availableRepresentatives]);
                updateSummaryLabel(finalSelection);
                resetTableView();
                updateFetchButtonState();
            });
