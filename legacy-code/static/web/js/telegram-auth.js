(function (global) {
    ...
    const STRINGS = {
        instruction: 'Відкрийте міні-застосунок Bulat+ у Telegram та підтвердьте вхід.',
    ...
    if (documentRef) {
        if (documentRef.readyState === 'loading') {
            documentRef.addEventListener('DOMContentLoaded', initializeGateway);
        } else {
            initializeGateway();
        }
    }
    ...
    function createTelegramAuth() {
        ...
        const authenticate = async () => {
            if (!rawInitData) {
                notify(`${STRINGS.notAuthorized} ${STRINGS.instruction}`, telegramApp ? 'warning' : 'error');
                throw new Error('Telegram init data is missing');
            }

            notify(STRINGS.progress);

            const response = await global.fetch('/api/auth/telegram', { method: 'POST' });
            const payload = await parseResponse(response);

            if (!response.ok) {
                const message = typeof payload === 'string' ? payload : payload?.error || STRINGS.genericError;
    ...
    function initializeGateway() {
        ...
        const botUsername = resolveBotUsername(ui.widgetContainer, appContext);
        ...
        const widget = createWidgetController({
            ...
            onError: error => {
                ...
                setMessage(ui.message,
                    'Не вдалося підтвердити авторизацію через Telegram. ' + STRINGS.instruction,
                    'error');
                setLoading(ui, false);
        ...
        function startAuthFlow() {
            if (!global.TelegramAuth) {
                setMessage(ui.message, 'Схоже, щось пішло не так. Оновіть сторінку або спробуйте пізніше.', 'error');
                setLoading(ui, false);
                return;
            }
            ...
            global.TelegramAuth.init({
                ...
                onError: error => {
                    ...
                    setMessage(ui.message, 'Не вдалося підтвердити авторизацію. ' + STRINGS.instruction, 'error');
                    setLoading(ui, false);
                }
            })
                .then(result => {
        ...
        function showAuthSuccess() {
            setMessage(ui.message, 'Авторизація успішна, відкриваємо головну…', 'success');
            setLoading(ui, true);
            redirectHome();
        }
    ...
    function sendWidgetAuthRequest(user) {
        return global.fetch('/api/auth/telegram/widget', {
            method: 'POST',
